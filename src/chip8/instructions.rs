...
impl Chip8 {
    pub(super) fn cls_00e0(&mut self) {
        self.video[..].fill(false);
    }

    pub(super) fn ret_00ee(&mut self) {
        if self.sp == 0 {
    ...
    pub(super) fn jp_1nnn(&mut self, nnn: u16) {
        self.pc = nnn;
    }

    pub(super) fn call_2nnn(&mut self, nnn: u16) {
    ...
    pub(super) fn se_3xkk(&mut self, x: u8, kk: u8) {
        if self.registers[x as usize] == kk {
            self.pc += 2;
        }
    }

    pub(super) fn sne_4xkk(&mut self, x: u8, kk: u8) {
        if self.registers[x as usize] != kk {
            self.pc += 2;
        }
    }

    pub(super) fn se_5xy0(&mut self, x: u8, y: u8) {
        if self.registers[x as usize] == self.registers[y as usize] {
            self.pc += 2;
        }
    }

    pub(super) fn ld_6xkk(&mut self, x: u8, kk: u8) {
        self.registers[x as usize] = kk;
    }

    pub(super) fn add_7xkk(&mut self, x: u8, kk: u8) {
        self.registers[x as usize] = self.registers[x as usize].overflowing_add(kk).0;
    }

    pub(super) fn ld_8xy0(&mut self, x: u8, y: u8) {
        self.registers[x as usize] = self.registers[y as usize];
    }

    pub(super) fn or_8xy1(&mut self, x: u8, y: u8) {
        self.registers[x as usize] |= self.registers[y as usize];
    }

    pub(super) fn and_8xy2(&mut self, x: u8, y: u8) {
        self.registers[x as usize] &= self.registers[y as usize];
    }

    pub(super) fn xor_8xy3(&mut self, x: u8, y: u8) {
        self.registers[x as usize] ^= self.registers[y as usize];
    }
    ...
    pub(super) fn ld_annn(&mut self, nnn: u16) {
        self.index = nnn;
    }

    pub(super) fn jp_v0_bnnn(&mut self, nnn: u16) {
        self.pc = self.registers[0] as u16 + nnn;
    }

    pub(super) fn rnd_cxkk(&mut self, x: u8, kk: u8) {
        let rnd: u8 = rand::random();

        self.registers[x as usize] = rnd & kk;
    }

    pub(super) fn drw_dxyn(&mut self, x: u8, y: u8, n: u8) {
    ...
    pub(super) fn skp_ex9e(&mut self, x: u8) {
    ...
    pub(super) fn sknp_exa1(&mut self, x: u8) {
        let key = self.registers[x as usize];
    ...
    pub(super) fn ld_fx07(&mut self, x: u8) {
        self.registers[x as usize] = self.delay_timer;
    }

    pub(super) fn ld_fx0a(&mut self, x: u8) {
        for i in 0..=16 {
            if i == 16 {
                self.pc -= 2;
                break;
